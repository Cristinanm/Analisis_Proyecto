...
function GestionUsuarios({ token, perfilActual }) {
  ...
  const cargarUsuarios = async () => {
    ...
    try {
      ...
      if (String(err.message).toLowerCase().includes("not found")) {
        setError(
          "Not Found: el backend no tiene cargadas las rutas admin. Reinicia backend en puerto 8000 y vuelve a intentar."
        );
      } else {
  ...
  const onSubmitModal = async (e) => {
    ...
    try {
      if (modoModal === "crear") {
        ...
        const actualizado = await updateUserByAdmin(
          token,
          usuarioEditando.id,
          payload
        );
  ...
  const manejarDesbloqueo = async (userId) => {
    setMensaje("");
    setError("");
    try {
      const actualizado = await unlockUser(token, userId);
      setUsuarios((prev) =>
        prev.map((u) => (u.id === actualizado.id ? actualizado : u))
      );
      setMensaje("Usuario desbloqueado correctamente.");
    } catch (err) {
      setError(err.message);
    }
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Administra cuentas, roles, estado de acceso y bloqueo por intentos fallidos.
          </p>
        </div>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {u.bloqueado && (
                        <button
                          type="button"
                          className="rounded-lg border border-amber-400/50 bg-amber-400/10 px-3 py-1.5 text-xs text-amber-200"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-xl rounded-2xl border border-zinc-700 bg-zinc-900 p-5 shadow-2xl">
            ...
            <form onSubmit={onSubmitModal} className="grid gap-3 md:grid-cols-2">
              ...
              {modoModal === "crear" && (
                <input
                  className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm md:col-span-2"
                  type="password"
              ...
              <div className="mt-2 flex gap-2 md:col-span-2">
                <button
                  type="submit"
                  disabled={guardando}
                  className="rounded-lg bg-amber-300 px-4 py-2 text-sm font-semibold text-zinc-950 disabled:opacity-60"
                >
                  {guardando
                    ? "Guardando..."
                    : modoModal === "crear"
